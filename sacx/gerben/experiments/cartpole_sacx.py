...
if __name__ == '__main__':
    ...
    q_network = QNetwork((4,), actions, tasks, ks.layers.Dense(100, activation='relu'),
                         ks.layers.Dense(2, activation='linear'), lambda x: x.state, gamma=0.9, alpha=0.001, fixed_steps=100)
    p_network = PolicyNetwork((4,), actions, tasks, ks.layers.Dense(100, activation='relu'),
                         ks.layers.Dense(2, activation='softmax'), lambda x: x.state, entropy_regularization=-5, alpha=0.001, fixed_steps=100)
