...
if __name__ == '__main__':
    ...
    q_network = QNetwork((2,), actions, tasks, common_net,
                         task_q_net, lambda s: s.state, gamma=0.9, alpha=0.01, reward_scale=1.0, fixed_steps=3)
    p_network = PolicyNetwork((2,), actions, tasks, common_net,
                         task_p_net, lambda s: s.state, entropy_regularization=0.05, alpha=0.001, fixed_steps=3)

    agent = SACU(env, q_network, p_network, tasks, num_learn=5, scheduler_period=200)
