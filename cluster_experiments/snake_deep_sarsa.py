...
def snake_deep_sarsa(run_num=0, file_name='snek'):
    import keras as ks
    import numpy as np
    ...
    env = SnakeContinuous(render=True, render_freq=10)
...
if __name__ == '__main__':
    jobs = [Process(target=snake_deep_sarsa,
                    args=(i, 'snake_continuous_deep_sarsa_run_' + str(i) + '.h5')) for i in range(NUM_RUNS)]
    for j in jobs:
        j.start()
    for j in jobs:
        j.join()
